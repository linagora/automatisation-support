...
function buildOpenRagPrompt(queryText: string): string {
  return [
    ...
    "Separate concise operational knowledge into:",
    "- customer-facing information that can be safely shown",
    "- questions a customer can reasonably answer",
    "- internal support/dev notes",
    "- limitations and claims to avoid",
    "- content that must not be exposed to the customer",
    "Do not mix customer actions with internal support/dev actions.",
    "",
    "Support issue:",
    queryText
  ].join("\n");
