...
import type {IssueFieldRequirement, IssueFieldSpec, SimilarIssueTopicCandidate} from "../runIssueResolutionBranch";
...
function getDeepIssueFieldSpecs(input: {
  ...
  if (input.selectedTopic && input.selectedTopic.usefulDeepFieldKeys.length > 0) {
    const validSelectedTopicFieldKeys = input.selectedTopic.usefulDeepFieldKeys
      .filter(isCaseDetailFieldKey)
      .filter(isAskableCaseDetailField)
      .map((key) => ({key, requirement: "required" as const}));

    if (validSelectedTopicFieldKeys.length > 0) {
      return resolveIssueFieldSpecs(validSelectedTopicFieldKeys);
    }
  }
